...
declare_id!("UfEzgTJzxjpGD7xou27ZvjrPxazeF1m4BVHMmMWdvmK");
...
pub mod solana_chainlink {
    ...
    pub fn execute(ctx: Context<Execute>) -> Result<()> {
        ...
        let description = chainlink::description(ctx.accounts.chainlink_program.to_account_info(), ctx.accounts.chainlink_feed.to_account_info())?;
        let decimals = chainlink::decimals(ctx.accounts.chainlink_program.to_account_info(), ctx.accounts.chainlink_feed.to_account_info())?;
